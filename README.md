...
<p>[2020/10/7追記] Python／folium／コロプレス図で色がつかない箇所があったので修正しました。<br>※KEYCODE1の「”数字”」→「数字(ダブルクオテーション無し)」へ修正。</p>

<p>①本境界データ②Excel（「KEYCODE1」「value」で表を作成したもの）を用いてPhthonでコロプレス図が書けます。</p>	
<p>以下「KEYCODE1」です。どうぞご利用ください。</p>
<ul>
...
<h2>作成のきっかけ</h2>
<p>子どもが多いエリアに保育園を誘致してほしいと思ったので、統計書で公開されている子どもの数をpythonで可視化してみようと思いました。区の丁々別の境界データ（geoJSON）が無かったため、試行錯誤で整形して作成しました。</p>
